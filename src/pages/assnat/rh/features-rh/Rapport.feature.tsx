        ...
        >
            ...
            <div className="h-screen overflow-y-auto text-gray-800 p-6">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-4 mb-8">
                    {leaves.map((item) => {
                        const isActive = item.label === "DEMANDES";
                        return (
                            <div
                                key={item.label}
                                onClick={() => {
                                    if (isActive && item.label === "DEMANDES") {
                                        navigate("demandes");
                                    }
                                }}
                                className={`bg-white border p-6 text-center transition ${
                                    isActive
                                        ? "border-gray-200 hover:border-emerald-600 hover:bg-emerald-50 cursor-pointer"
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-4">
                    {presence.map((item) => {
                        const isActive = item.label === "RÈGLEMENTS";
                        return (
                            <div
                                key={item.label}
                                onClick={() => {
                                    if (isActive && item.label === "RÈGLEMENTS") {
                                        navigate("reglements");
                                    }
                                }}
                                className={`bg-white border p-6 text-center transition ${
