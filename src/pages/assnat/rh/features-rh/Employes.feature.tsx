...
import DrawerContratPaie from "../../../../components/rh/Drawer-contrat-paie";
import DrawerDocumentPersonnel from "../../../../components/rh/Drawer-document-personnel";
...
const EmployesFeatures: React.FC = () => {
    ...
    const [isOpenContratPaie, setIsOpenContratPaie] = useState(false);
    const [isOpenDocument, setIsOpenDocument] = useState(false);
    const [contratPaieMode, setContratPaieMode] = useState<"contrat" | "paie">("contrat");
    ...
    const [personnelToEdit, setPersonnelToEdit] = useState<PersonnelSummary | null>(null);
    const [loading, setLoading] = useState(true);
    const [error, setError] = useState<string | null>(null);

    const { getAllPersonnel, getAllServices, getPersonnelById } = useRhService();
    ...
    return (
        ...
        >
            ...
            <div className="px-6 py-2 flex items-center justify-between">
                <span className="text-sm text-gray-500">{employesCountLabel}</span>
                <button
                                    ...
                                    <Methode 
                                        onEdit={async () => {
                                            try {
                                                ...
                                                const fullPersonnel = await getPersonnelById(personnel.id_personnel);
                                                setPersonnelToEdit(fullPersonnel as PersonnelSummary);
                                                setIsOpen(true);
                                            } catch (err: any) {
                                        ...
                                        onContrat={() => {
                                            setSelectedPersonnel(personnel);
                                            setContratPaieMode("contrat");
                                            setIsOpenContratPaie(true);
                                        }}
                                        onPaie={() => {
                                            setSelectedPersonnel(personnel);
                                            setContratPaieMode("paie");
                                            setIsOpenContratPaie(true);
                                        }}
                                        onDocument={() => {
                                            setSelectedPersonnel(personnel);
            ...
            <DrawerAddPersonne
                ...
                onCreated={async (personnel: any) => {
                    if (personnel && personnel.id_personnel) {
                        try {
                            // Récupérer les données complètes de l'employé pour avoir toutes les informations
                            const fullPersonnel = await getPersonnelById(personnel.id_personnel);
                            
                            // Formater les données pour correspondre au type PersonnelSummary
                            const personnelSummary: PersonnelSummary = {
                                id_personnel: fullPersonnel.id_personnel || personnel.id_personnel,
                                nom_personnel: fullPersonnel.nom_personnel || personnel.nom_personnel || "",
                                prenom_personnel: fullPersonnel.prenom_personnel || personnel.prenom_personnel || "",
                                email_travail: fullPersonnel.email_travail || personnel.email_travail,
                                role_personnel: fullPersonnel.role_personnel as RolePersonnel || personnel.role_personnel as RolePersonnel,
                                type_personnel: fullPersonnel.type_personnel as TypePersonnel || personnel.type_personnel as TypePersonnel,
                                id_service: fullPersonnel.id_service || personnel.id_service,
                                is_active: fullPersonnel.is_active !== undefined ? fullPersonnel.is_active : personnel.is_active,
                            };

                            if (personnelToEdit) {
                                // Mode édition : mettre à jour la liste
                                setPersonnels((prev) => prev.map((p) => 
                                    p.id_personnel === personnelSummary.id_personnel ? personnelSummary : p
                                ));
                                // Mettre à jour aussi selectedPersonnel si c'est le même employé
                                if (selectedPersonnel?.id_personnel === personnelSummary.id_personnel) {
                                    setSelectedPersonnel(personnelSummary);
                                }
                                setPersonnelToEdit(null);
                            } else {
                                // Mode création : ajouter à la liste (éviter les doublons)
                                setPersonnels((prev) => {
                                    const filtered = prev.filter((p) => p.id_personnel !== personnelSummary.id_personnel);
                                    return [personnelSummary, ...filtered];
                                });
                                setSelectedPersonnel(personnelSummary);
                                setActive(personnelSummary.id_personnel);
                                setIsOpenConsultez(true);
                            }
                        } catch (err: any) {
                            console.error("Erreur lors de la récupération des données complètes:", err);
                            // En cas d'erreur, utiliser les données partielles
                            const personnelSummary: PersonnelSummary = {
                                ...
                                role_personnel: personnel.role_personnel as RolePersonnel,
                                type_personnel: personnel.type_personnel as TypePersonnel,
            ...
            <DrawerSeePersonneData
                ...
                onPersonnelUpdated={(updatedPersonnel) => {
                    // Mettre à jour la liste après modification dans le drawer de consultation
                    if (updatedPersonnel?.id_personnel) {
                        const personnelSummary: PersonnelSummary = {
                            id_personnel: updatedPersonnel.id_personnel,
                            nom_personnel: updatedPersonnel.nom_personnel || "",
                            prenom_personnel: updatedPersonnel.prenom_personnel || "",
                            email_travail: updatedPersonnel.email_travail,
                            role_personnel: updatedPersonnel.role_personnel as RolePersonnel,
                            type_personnel: updatedPersonnel.type_personnel as TypePersonnel,
                            id_service: updatedPersonnel.id_service,
                            is_active: updatedPersonnel.is_active,
                        };
                        
                        setPersonnels((prev) => prev.map((p) => 
                            p.id_personnel === personnelSummary.id_personnel 
                                ? personnelSummary 
                                : p
                        ));
                        setSelectedPersonnel(personnelSummary);
            ...
            />
            <DrawerContratPaie
                isOpen={isOpenContratPaie}
                onClose={() => setIsOpenContratPaie(false)}
                idPersonnel={selectedPersonnel?.id_personnel || ""}
                nomPersonnel={selectedPersonnel?.nom_personnel}
                prenomPersonnel={selectedPersonnel?.prenom_personnel}
                mode={contratPaieMode}
                onCreated={() => {
                    // Rafraîchir les données si nécessaire
                    fetchData(true);
                }}
            />
            <DrawerDocumentPersonnel
...
function Methode({ onEdit, onContrat, onPaie, onDocument }: { onEdit: () => void; onContrat: () => void; onPaie: () => void; onDocument: () => void }) {
    return (
