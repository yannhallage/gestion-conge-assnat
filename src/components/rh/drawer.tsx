...
// import '@pdftron/pdfjs-express/dist/pdfjsexpress.css';
...
export default function Drawer({ isOpen,
    ...
    return (
        <>
            ...
            <div
                className={`fixed top-0 right-0 h-full w-[80%] bg-white shadow-xl z-50 transform transition-transform ${isOpen ? "translate-x-0" : "translate-x-full"
                    }`}
            >
...
function PDFViewer() {
    ...
    useEffect(() => {
        if (viewerRef.current) {
            WebViewer(
                {
                    path: "/webviewer", // dossier public contenant les assets PDF.js Express
                    initialDoc: "../../../public/webviewer/fiche-technique_licence.pdf", // PDF à afficher
                    zoom: "200%" 
                },
                viewerRef.current
            ).then((instance : any) => {
                // instance est le WebViewer
                console.log("PDF.js Express loaded");
            });
        }
    }, []);

    return <div ref={viewerRef} style={{ height: "500px" }} />;
}